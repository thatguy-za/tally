...
export const actions = {
  addCategory: async ({ request, locals }) => {
    ...
    let created;
    try {
      created = createCategory(locals.user.id, name, kind, color);
    } catch {
  ...
  suggest: async ({ locals }) => {
    ...
    try {
      const r = await suggestNewCategories(locals.user.id);
  ...
  addSuggested: async ({ request, locals }) => {
    ...
    for (const p of Array.isArray(picks) ? picks : []) {
      ...
      if (!name) continue;
      try {
        createCategory(locals.user.id, name, p?.kind || 'expense', p?.color);
        added++;
      } catch {
        /* skip duplicates */
      }
